...
A physical address \index{physical address} indicates to the hardware where a
particular byte in memory resides -- whether it is in RAM (a particular DIMM,
bank, row, and column), or in a portion of memory assigned to system
configuration
(e.g. \href{https://en.wikipedia.org/wiki/Advanced_Configuration_and_Power_Interface}{ACPI} or
\href{https://en.wikipedia.org/wiki/Advanced_Programmable_Interrupt_Controller}{APIC})
, memory-mapped device registers (portions of a device's memory that are
directly accessable by the system -- the system will route these memory accesses
over the bus to the appropriate hardware, e.g. the
